# Writing System - 종합 가이드

AI 기반 글쓰기 서브에이전트 시스템의 설치, 설정, 사용 방법을 안내합니다.
...
1. **전문화된 에이전트 분업**
   - 단일 LLM: "글 써줘" → 바로 결과물
   - AI Writing: 기획 → 스타일 분석 → 작성 → 검토 → 교정 → 학습
...
- **AI Writing**: 지속적으로 글을 쓰고, 일관된 브랜드 보이스가 필요하며, 글쓰기 실력을 발전시키고 싶은 사람에게 적합
...
Atlassian Cloud에서 **SSO(Atlassian Access)**가 활성화된 환경에서는 API 토큰 방식이 제한됩니다.
**OAuth 2.0 연동**을 통해 Confluence 문서를 동기화할 수 있습니다.
...
1. 브라우저에서 Confluence 스페이스 접속 (SSO 로그인)
2. 참고할 페이지 열기
...
# 2. 스페이스 검색 (키워드로)
python oauth_confluence.py --find "YOUR_KEYWORD"

# 3. 전체 스페이스 목록 조회
python oauth_confluence.py --spaces

# 4. 특정 스페이스 동기화
python oauth_confluence.py --sync --space "YOUR_SPACE"
...
├── 736988863_Project Overview.md   # 개별 페이지들
├── 736988864_기술 스택.md
└── ...
```
...
Claude/Gemini에서 활용:
```
"Confluence 캐시에서 관련 문서 찾아서 참고해줘"
...
```powershell
# 매일 오전 9시 동기화 (예시)
$action = New-ScheduledTaskAction -Execute "python" -Argument "oauth_confluence.py --sync --space YOUR_SPACE" -WorkingDirectory "D:\PilotProject\AgentWriting\writing-system\integrations\confluence"
